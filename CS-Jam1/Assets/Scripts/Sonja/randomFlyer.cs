...
// Requires animator with parameter 'flySpeed' catering for 0,1 (idle, flap)
[RequireComponent(typeof(Animator))]

// Requires rigidBody to move around
...
public class randomFlyer : MonoBehaviour
{
    ...
    private float   changeAnim = 0f,
                    ...
                    prevAnim,
                    currentAnim = 0f,
                    prevSpeed, speed, zturn, prevz,
                    turnSpeedBackup;
    private Vector3 rotateTarget, position, direction, velocity, randomizedBased;
    ...
    {
        // Wait if start should be delayed (useful to add small differences in lage flocks)
        if (delayStart > 0f)
        {
        ...
        // Calculate distances
        distanceFromBase = Vector3.Magnitude(randomizedBased - body.position);
        distanceFromTarget = Vector3.Magnitude(flyingTarget.position - body.position);

        // Allow trastic turns close to base to ensure target can be reached
        ...
        {
            if (body.transform.position.y < yMinMax.x + 10f) rotateTarget.y = 1f;
